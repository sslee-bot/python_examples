...
class FIR_partial:
    def __init__(self, f, h, state_set, input_set, N, w, K, x_init=None, z_init=None, u_init=None):
        # Scalar values
        self.dim_x = len(f)
        self.dim_z = len(h)
        self.dim_u = len(input_set)
    ...
    def C_big(self):    # caution: different with that of FIR
        output = np.zeros((self.dim_x, (self.N-1) * self.dim_x))
        for i in range(self.N-1):
            col_start = self.dim_x * i
            col_end = self.dim_x * (i + 1)
            output[:, col_start:col_end] = self.Mat_F(i + 1, 1)
    ...
    def estimate(self, z, u, alpha=True):
        ...
        Omega = np.concatenate((self.w * np.transpose(F_0), A_tran @ np.linalg.inv(self.K)), axis=1)
        L = self.F_array[-1] @ F_0 @ np.linalg.inv(Omega @ np.transpose(Omega)) @ Omega
        Lx = L[:, 0:self.dim_x]
        Ly = L[:, self.dim_x:] @ np.linalg.inv(self.K)
        M = np.concatenate(((self.F_array[-1] - self.w * Lx) @ C, np.eye(self.dim_x)), axis=1) - Ly @ B

        z_tilde_vector = np.reshape(self.z_tilde_array, (1, -1))[0]
        u_tilde_vector = np.reshape(self.u_tilde_array, (1, -1))[0]
        self.x_hat = self.w * Lx @ self.x_hat + Ly @ z_tilde_vector + M @ u_tilde_vector
        return self.x_hat
